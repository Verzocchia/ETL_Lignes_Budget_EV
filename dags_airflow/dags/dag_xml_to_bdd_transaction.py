    ...
    if comptage_presence[0] > 0 : 
     ...
     try : 
      ...
      try :
       ...
       logger.error(f"Erreur envoie data / annexe {id_fichier}: {e}")
# os.remove(file)
        
     except Exception as e : 
       logger.error(f"Erreur fichier {id_fichier}: {e}")
...
with DAG(dag_id="xml_to_bdd_transaction", start_date=datetime(2022, 4, 2)) as dag:
   dl_files_from_s3()
   operate_file.expand(file=get_files())

#def xml_to_bdd_transaction():
#    dl_task = dl_files_from_s3()
#    get_task = get_files()
#    
#    operate_tasks = [operate_files(file) for file in get_task]
#    
#    dl_task >> get_task >> operate_tasks
#
#dag = xml_to_bdd_transaction()